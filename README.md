...
典型的珍珠炮流程是:主世界蓄力 → 玩家載入區塊放行 → 珍珠穿地獄門 → 在地獄飛行。
地獄門那一步有個陷阱,實測(Paper 26.1.2)結果如下。

### 傳送門判定是「位置」不是「路徑」

```
# 門蓋在 x=100,珍珠從 x=0.5 以 1750 格/tick 起跑
T1  x=   0.50
T2  x=1733.00      ← 一 tick 跳過去,完全沒有 PORTAL 事件
```

珍珠只在「移動後的位置」與傳送門方塊重疊時才會被傳送。門是 1 格厚,而高速珍珠
一步跨上千格,所以會直接穿過去。慢速珍珠(0.5 格/tick)則正常傳送。

### 解法:把門蓋在算出來的落點上

彈道是決定性的,所以落點可以精確算出來。把門蓋在某個 tick 的落點:

```
# 門改蓋在 x=1733(= 第 1 tick 的落點)
PORTAL! world 1733.0,201.0,0.5 -> world_nether 216.0,200.0,0.0  motion=(1732.500, -0.030, 0.000)
```

### 動量跨維度完整保留

上面那行的 `motion=(1732.500, ...)` 就是證據:**1732.5 的動量穿過地獄門之後一格不差**,
沒有被 `Entity.load` 的 >10 clamp 砍掉。傳送門走的不是存檔讀取那條路徑。

### 但抵達座標會被吸附

`1733 / 8 = 216.625`,實際抵達的是 `216.0, 200.0, 0.0`——x/y/z 都被吸附到方塊位置,
而且如果附近已經有地獄門,珍珠會被送到那座門而不是換算座標。所以**地獄段的彈道要用
實際抵達座標重新起算**,不能直接把主世界的軌跡除以 8。
